...
## Co-op
The game itself is used to swap between the two characters in order to clear some obstacles, but in this build if the user hits ENTER or START on their controller, Kinder Milchschnitte (called Fettalatte in Italy) will spawn and can be controlled by another player. Ammo and health are shared between both characters. Since Milchschnitte can't float, you can use T or LB to instantly teleport near Kinder Pinguì.
...
At the moment no in-game options has been made for this, but it'll pick whichever audio language is available. If both are available at the same time, Italian takes priority.

## Console ports 
The code was made with portability in mind. Any console that isn't super hideous to work with is absolutely capable of running this game.
