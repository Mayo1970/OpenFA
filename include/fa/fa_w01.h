/*
 * fa_w01.h - .W01 sprite / background container + frame decode
 *
 * A direct port of the reverse-engineered readers:
 *   - the 1063-byte header and the 22-byte chained frame directory
 *     (W01_GetFileHeader fcn.0042bb1f, PrivateLoadW01 fcn.0042c6c1)
 *   - the per-frame pixel decode: store / RLE8 / RLE16, little-endian RGB565
 *     (PrivateReadImage fcn.0042c96c). Every shipped frame is bpp 16, comp 2
 *     (RLE16).
 *
...
typedef struct fa_w01_frame {
    ...
    uint16_t rect[4];         /* +0x0E crop/origin rect (editor data)        */
 ...
 * table B. Table B holds the frame-record offsets. Table A is read by
 * W01_LoadKorrektur into the per-frame origin the front-end positions sprites
 * with: each u32 = (x | (y << 16)) in screen pixels - confirmed against the
 * menu layout (GIUNGLA 37,96; VALLE 8,218; MONTAGNA 12,342; FABBRICA 56,464;
 * CLASSIFICA 523,535; ESCI 700,531).
 */
