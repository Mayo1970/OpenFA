/*
 * fa_player.h - the player controller
 *
 ...
 *   fcn.00417370 (state machine) + fcn.00431bf0 (integrator):
 ...
 *   constant, so the player uses it too. The binary constants are the source;
 ...
 *   a raised surface Y under a world X - kept for the headless jump-arc
 *   tuning aid and the no-GData tests.
 */
#ifndef FA_PLAYER_H
#define FA_PLAYER_H
...
typedef struct fa_player_tuning {
    ...
    int32_t  snow_gravity;      /* + per tick (1.0)                          */
    int32_t  snow_term_vy;      /* terminal vy clamp (20.0)                  */
    int      snow_life;         /* ticks before it despawns                  */
    int32_t  snow_off_x, snow_off_y;   /* spawn offset from the player       */

    /* collision box for fa_player_set_solid. First pass - no exe symbol
     * pins it; tunable like the physics constants. */
...
#define FA_MAX_SNOWBALLS 10   /* the original pool is 10 */
...
typedef struct fa_player {
    ...
    /* idle: a sub-mode of STAND, not a state */
    ...
    /* push: state 33 is a committed 172..190 clip, ~38 ticks; the
    ...
    void   *ground_ctx;
    /* map collision probe: the solidity class at world pixel (px, py) -
     * 0 none / 1 solid / 2 one-way. NULL = flat-floor fallback. */
    ...
    uint32_t rng_state;      /* xorshift32 - idle rolls                       */
    uint64_t tick;
...
/* Map collision. `fn` returns the solidity class at a world pixel
