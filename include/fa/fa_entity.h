/*
 * fa_entity.h - level object / sprite layer
 *
 ...
 * missing its sprites.
 *
 * Reverse-engineered from JR_FERRERO.exe. Summary:
 ...
 *
 * WHAT THIS MODULE DOES: parse the tail, resolve every placement to its
 * sprite, draw it in the right bucket order, and run the generic per-object
 * runtime every simulation tick:
 ...
 *     there, then deactivates the marker.
 ...
 *     report a hit.
 *
 * WHAT IT DEFERS TO THE BEHAVIOUR LAYER (fa_beh.c): the per-ObjNr callbacks
 * the exe installs at record +0x5E (fcn.004311C0 / the ~60 calls at
 * 0x4119F7..) - enemy AI, Attack/Freeze/KO state switching,
 * projectile <-> enemy, the exact lift-landing sub-branches (jump table
 * 0x412BD4), and pushables. fa_entity_set_behaviour() is the seam those bind
 * through.
 */
#ifndef FA_ENTITY_H
#define FA_ENTITY_H
...
typedef struct fa_entity_rec {
    ...
    /* --- runtime state (advanced by fa_entity_tick) ------------------- */
    ...
    /* per-object behaviour scratch, owned by fa_beh.c. The exe keeps
...
/* Mutable access to the i-th record, for the behaviour layer. */
 ...
 * per-ObjNr AI gives each enemy type its real movement.
 */
 ...
 * Returns 1 and fills x and y if the marker is present, else 0.
 ...
 * The stomp-vs-damage resolution lives in fa_beh.c.
 */
