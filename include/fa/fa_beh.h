...
/* Per-tick player view used by the enemy layer. Resources remain owned by the
 * host; this only publishes positions, vulnerability and live snowballs. */
typedef struct fa_beh_player {
    int active;
    int feet_x, feet_y;
    int half_w, height;
    int facing;
...
/* Publish up to two players. Enemy attacks and contact tests choose the
 * nearest active player, with player 0 winning exact-distance ties. */
 ...
 * terrain-kill, player damage). Returns the total damage generated this tick
 * (0 or 20 in normal use); *knockback is set to 1 when a contact happened.
 * In a two-player publish, use the per-player accessors below to identify the
 * target.
 */
int fa_beh_post(fa_beh *b, int *knockback);

/* Damage generated by the most recent fa_beh_post(), per player. */
