/*
 * fa_surface.h - RGB565 software surface and blitter
 *
 * The original composites every frame with DirectDraw `BltFast` on 16 bpp
 * RGB565 surfaces, back to front, with no hardware clipper and no scaling.
 * This module reproduces that on a portable software target.
 *
 *   Pixel format   RGB565, R 0xF800 / G 0x07E0 / B 0x001F, no alpha.
 *   Surface        a pixel plane plus a byte pitch. Sprite pitch is w*2
 *                  ROUNDED UP to an alignment (96 px -> 192, 122 px -> 248);
 *                  the blitter never assumes pitch == w*2.
...
/* Default source colour key: black, exact match. */
#define FA_COLORKEY    0x0000u

/* Default surface row alignment in bytes (pitch is a multiple of 8). */
...
/* Expand an RGB565 value to 8:8:8, bit-replicated. */
 ...
 * .W01 frame) into `dst`, which must be at least src_w x src_h.
 ...
 * exactly `key` is skipped (DDCKEY_SRCBLT exact match).
...
/*
 * Colour fill (DDBLT_COLORFILL): set every pixel of `rect`
